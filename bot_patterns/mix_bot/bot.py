...
subprocess.Popen(["python.exe", f"{path}/mixer.py"])
...
while True:#бесконечный 4loop. чтобы, если какая-то ошибка, то бот перезапускался
    ...
    ################################################################################################################
    ######################################## ЧТО В РАЗДЕЛЕ МИКСЕР ##################################################

                    elif message.text.startswith('миксер'):
                        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
                        groups2mix = types.KeyboardButton('⤴️\nотсюда')
                        options = types.KeyboardButton('⚙️\nопции')
                        group2send = types.KeyboardButton('⤵️\nсюда')
                        menu = types.KeyboardButton('меню 🔙')

                        markup.add(groups2mix, options, group2send, menu)
                        bot.send_message(message.chat.id, '⌨️', reply_markup=markup) 

                        clear()

                    elif message.text == ('⤵️\nсюда'):
                        if os.path.getsize('sender/to_here/to_here.txt') == 0: #если не указана ещё группа, куда отправлять посты
                            # функции по обработке +группа
                            @bot.message_handler(func=lambda message: True) # 'скинь ссылку на группу' + функция по обратоке ссылки
                            def url_plus(message):
                                sent = bot.send_message(message.chat.id, '<em>скинь ссылку на группу</em>', parse_mode='html')
                                bot.register_next_step_handler(sent, save_link)

                            def save_link(message): #обработка ссылки и добавление её в отправку
                                publick_link = message.text
                                if publick_link.startswith('https://vk.com/'): #если ссылка корректна
                                    if pub_checker(publick_link) != 100 : #проверить, существует ли вообще такая группа
                                        with open('sender/to_here/to_here.txt', 'w', encoding='utf-8') as file:
                                            file.write(publick_link) #добавление ссылки на группу в отправку
                                            file.close()
                                        bot.send_message(message.chat.id, "<em>группа добавлена в отправку!</em>", parse_mode='html')
                                        if os.path.exists(f"sender/{publick_link.replace('https://vk.com/', '')}.txt"):
                                            print_time_now()
                                            print(f'добавлена повторно группа для отправки: {publick_link}')
                                        else:
                                            new_group = open(f"sender/{publick_link.replace('https://vk.com/', '')}.txt", "w", encoding='utf-8') #создать txt с id группы
                                            new_group.write('0')
                                            new_group.close()
                                            print_time_now()
                                            print(f'добавлена группа для отправки: {publick_link}')
                                    else:
                                        bot.send_message(message.chat.id, "с группой что-то не так...\nпожалуйста, проверь, всё ли правильно и попробуй снова")
                                        url_plus(message)
                                elif publick_link.startswith('меню 🔙') or publick_link.startswith('⤴️\nотсюда') or publick_link.startswith('⚙️\nопции') or publick_link.startswith('⤵️\nсюда'):
                                    pass
                                else: #если ссылка некорректна
                                    bot.send_message(message.chat.id, "пожалуйста, отправь ссылку в формате: \nhttps://vk.com/id_паблика\n\n(если ты передумал, нажми на любую кнопку)")
                                    url_plus(message)
                        ...
                        else: #если уже указана группа
                            f = open('sender/to_here/to_here.txt', 'r', encoding='utf-8')
                            line = f.readline()
                            f.close()

                            markup = types.InlineKeyboardMarkup(row_width=1) #добаление кнопок под группой
                            delete = types.InlineKeyboardButton('убрать из отправки', callback_data=f'del_to_here')
                            markup.add(delete)
                            publos = bot.send_message(message.chat.id, line, reply_markup=markup, disable_web_page_preview=True) #отправка паблика
                            publos_id.append(publos.id) #добавление id сообщения с пабликом

                            markup = types.ReplyKeyboardMarkup(resize_keyboard=True) #если ничего не надо делать со списком пабликов
                            sender = types.KeyboardButton('миксер 🔙')
                            markup.add(sender)
                            mini_helper = bot.send_message(message.chat.id, '(если не хочешь менять группу для отправки постов, переходи назад)', reply_markup=markup)
                            helper.append(mini_helper.id)
                        ...
                        if os.path.getsize('publicks/publicks.txt') == 0:
                            ...
                            number = 0
                            f = open('publicks/publicks.txt', 'r', encoding='utf-8')
                            list_of_pub = f.readlines()
                            f.close()
                            for line in list_of_pub:
                                f = open('sender/from_here/from_here.txt', 'r', encoding='utf-8')
                                for_check = f.readlines()
                                f.close()
                                if for_check == []:
                                    markup = types.InlineKeyboardMarkup(row_width=1) #добаление кнопок под каждым пабликом
                                    add = types.InlineKeyboardButton('не в миксере 🅾️', callback_data=f'add_from_here') #добавить паблик в список
                                    markup.add(add)
                                    publos = bot.send_message(message.chat.id, line, reply_markup=markup, disable_web_page_preview=True) #отправка паблика
                                    publos_id.append(publos.id) #добавление id сообщения с пабликом
                                else:
                                    this = line.partition(': ')[2]
                                    in_mixer = False
                                    ...
                                    if in_mixer:
                                        markup = types.InlineKeyboardMarkup(row_width=1) #добаление кнопок под каждым пабликом
                                        delete = types.InlineKeyboardButton('в миксере ❇️', callback_data=f'del_from_here') #удалить паблик из миксера
                                        markup.add(delete)
                                        publos = bot.send_message(message.chat.id, line, reply_markup=markup, disable_web_page_preview=True) #отправка паблика
                                        publos_id.append(publos.id) #добавление id сообщения с пабликом
                                    else:
                                        markup = types.InlineKeyboardMarkup(row_width=1) #добаление кнопок под каждым пабликом
                                        add = types.InlineKeyboardButton('не в миксере 🅾️', callback_data=f'add_from_here') #добавить паблик в миксер
                                        markup.add(add)
                                        publos = bot.send_message(message.chat.id, line, reply_markup=markup, disable_web_page_preview=True) #отправка паблика
                                        publos_id.append(publos.id) #добавление id сообщения с пабликом
                            ...
                            sender = types.KeyboardButton('миксер 🔙')
                            markup.add(sender)
                            mini_helper = bot.send_message(message.chat.id, '(Если никак не хочешь изменить список миксера, переходи назад)', reply_markup=markup)
                            helper.append(mini_helper.id)

                    elif message.text == '⚙️\nопции': #контент, который будет миксоваться
                        markup = types.InlineKeyboardMarkup(row_width=2) #добаление кнопок под сообщением
                        photo = types.InlineKeyboardButton('фото', callback_data='photo') 
                        photo_count = types.InlineKeyboardButton('кол-во фото', callback_data='photo_count') #изменить кол-во фото
                        music = types.InlineKeyboardButton('музыка', callback_data='music') 
                        video = types.InlineKeyboardButton('видео', callback_data='video') 
                        markup.add(photo, photo_count, music, video)
                        f = open('mixer/photo_switcher.txt', 'r', encoding='utf-8')
                        pht = f.readline()
                        f.close
                        f = open('mixer/music_switcher.txt', 'r', encoding='utf-8')
                        mus = f.readline()
                        f.close
                        f = open('mixer/video_switcher.txt', 'r', encoding='utf-8')
                        vid = f.readline()
                        f.close
                        ...
                        if vid == 'on':
                            ...
                            vid = ''
                        bot.send_message(message.chat.id, f'Какой контент будет миксоваться для отправки?\nСейчас миксуется: <b>{pht}{ph_count} {mus} {vid}</b>\n\n'+
                        '( контент миксуется, формируется и отправляется автоматически по мере появления новых постов из монитора )', parse_mode='html', reply_markup=markup, disable_web_page_preview=True) #отправка паблика
            ...
            try:
                if call.message:
                    ...
                    def clear(message, txt):
                        ...
                        bot.edit_message_text(chat_id=chat__id, message_id=message.message_id, text=message.text + f'\n✖️<s>убран из {txt}</s>', parse_mode='html', reply_markup=None, disable_web_page_preview=True) #изменение ссылки паблика 

                        what_deleting = 0 #id сообщения с пабликом, которое удаляли = 0 (на всякий случай)
                        publos_id.clear() #очищение списка id сообщений с пабликами
                        helper.clear() #очишение списка вспомогательных сообщений

                    def clear_added(message, txt):
                        f = open("groups_processors/chat__id.txt", "r", encoding='utf-8')
                        chat__id = f.readline()
                        f.close()
                        for ids in publos_id:
                        ...
                        bot.edit_message_text(chat_id=chat__id, message_id=message.message_id, text=message.text + f'\n➕<em>добавлен в миксер</em>', parse_mode='html', reply_markup=None, disable_web_page_preview=True) #изменение ссылки паблика 

                        what_deleting = 0 #id сообщения с пабликом, которое удаляли = 0 (на всякий случай)
                        publos_id.clear() #очищение списка id сообщений с пабликами
                        helper.clear() #очишение списка вспомогательных сообщений

                    if call.data == 'count': #если хочет изменить кол-во сообщений для ведомлений
                    ...
                    elif call.data == 'photo': #настройка миксера
                        if os.path.getsize('mixer/photo_switcher.txt') != 0:
                            f = open('mixer/photo_switcher.txt', 'w', encoding='utf-8')
                            f.close()
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер фото выключен', parse_mode='html')
                        else:
                            f = open('mixer/photo_switcher.txt', 'w', encoding='utf-8')
                            f.write('on')
                            f.close
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер фото включён', parse_mode='html')
                    elif call.data == 'music': #настройка миксера
                        if os.path.getsize('mixer/music_switcher.txt') != 0:
                            f = open('mixer/music_switcher.txt', 'w', encoding='utf-8')
                            f.close()
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер музыки выключен', parse_mode='html')
                        else:
                            f = open('mixer/music_switcher.txt', 'w', encoding='utf-8')
                            f.write('on')
                            f.close
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер музыки включён', parse_mode='html')
                    elif call.data == 'video': #настройка миксера
                        if os.path.getsize('mixer/video_switcher.txt') != 0:
                            f = open('mixer/video_switcher.txt', 'w', encoding='utf-8')
                            f.close()
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер видео выключен', parse_mode='html')
                        else:
                            f = open('mixer/video_switcher.txt', 'w', encoding='utf-8')
                            f.write('on')
                            f.close
                            bot.send_message(chat_id=call.message.chat.id, text='Миксер видео включён', parse_mode='html')
                    ...
                    #если кнопка "удалить группу для отправки поста to_here"
                    elif call.data == 'del_to_here':

                        f = open("sender/to_here/to_here.txt", "r", encoding='utf-8')
                        group_name = f.readline()
                        f.close()
                        print_time_now()
                        print(f'удалил паблик to_here: {group_name}')

                        #убираем строку из файла to_here
                        f = open('sender/to_here/to_here.txt', 'w', encoding='utf-8')
                        f.close()
                        
                        text = 'рассылки'
                        clear(call.message, text)

                    #если кнопка "добавить группу в список миксера from_here"
                    elif call.data == 'add_from_here':
                        publick_link = call.message.text
                        publick_link = publick_link.partition(': ')[2]

                        f = open('sender/from_here/from_here.txt')
                        check = f.readlines()
                        f.close
                        print_time_now()
                        if check == []: #добавляется впервые
                            print(f'первый раз добавлен в миксер: {publick_link}')
                        else:
                            print(f'паблик добавлен в миксер: {publick_link}')

                        how_many_lines = sum(1 for line in open('sender/from_here/from_here.txt', 'r', encoding='utf-8')) + 1
                        f = open('sender/from_here/from_here.txt', 'a', encoding='utf-8')
                        f.write(str(how_many_lines) + ": " + publick_link + "\n") #добавление ссылки в миксер

                        text = 'миксер'
                        clear_added(call.message, text)

                    #если кнопка "удалить группу из списка миксера from_here"
                    elif call.data == 'del_from_here':
                        this_delete = call.message.text
                        print(f'удалил паблик из миксера: {this_delete.partition(": ")[2]}')
                        this_delete += '\n'


                        #убираем строку из списка from_here
                        f = open("sender/from_here/from_here.txt", "r", encoding='utf-8')
                        lines = f.readlines()
                        f.close()
                        f = open("sender/from_here/from_here.txt", "w", encoding='utf-8')
                        for line in lines:
                            if line.partition(': ')[2]!=this_delete.partition(': ')[2] :
                                f.write(line)
                        f.close()
                        #переделываем список from_here
                        f = open("sender/from_here/from_here.txt", "r", encoding='utf-8') 
                        lines = f.readlines()
                        f.close()
                        f = open("sender/from_here/from_here.txt", "w", encoding='utf-8')
                        ...
                        for line in lines:
                            remake = ''
                            if int(line.partition(': ')[0]) == count:
                                f.write(line)
                                count += 1
                            elif int(line.partition(':')[0]) > count:
                                remake = remake + str(count)
                    ...
                    for num in range(1000):
                        ...
                        #если кнопка "удалить пост из списка монитора"
                            ...
                            #также удаляем из списка миксера
                            f = open('sender/from_here/from_here.txt', 'r', encoding='utf-8')
                            spisok = f.readlines()
                            f.close
                            f = open('sender/from_here/from_here.txt', 'w', encoding='utf-8')
                            finder = False
                            for line in spisok:
                                num = line.partition(':')[0]
                                line = line.partition(':')[2]
                                line = line.replace(' https://vk.com/', '')
                                line = line.replace('\n', '')
                                if line == group_name:
                                    line = ''
                                    f.write(line)
                                    ...
                                    line = num + ': https://vk.com/' + line + '\n'
                                    f.write(line)
                                elif finder:
                                    num = int(num) - 1
                                    line = str(num) + ': https://vk.com/' + line + '\n'
                                    f.write(line)
                            f.close()

                            #bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<s>убран из монитора</s>', parse_mode='html', reply_markup=None) #изменение ссылки паблика 
                            text = 'монитора'
                            clear(call.message, text)
